...
Read this only when Ratel (`@ratel-ai/sdk`, `ratel-ai-core`, `@ratel-ai/mcp-server`, or `ratel-mcp`) is present in the codebase, or the customer is signing up to add it during the engagement. Otherwise skip — don't put Ratel scaffolding into customer code unless they're actually using or adopting it.

## What Ratel emits

Ratel's core (`src/core/lib/src/trace/`) emits a typed stream of `TraceEnvelope` events. The variants the SDK forwards to Langfuse:

| Ratel event | Langfuse mapping | Observation name | `metadata` keys |
| --- | --- | --- | --- |
| `Search` | `type: tool` | `ratel.search_tools` | `query`, `top_k`, `hit_count`, `took_ms`, `gateway_origin` (`direct` or `agent`), `top_hit_score` |
...
The `@ratel-ai/sdk` and forthcoming Python binding will, in v0.1.5+, expose a Langfuse sink that adapts the `TraceEnvelope` stream into Langfuse observations. Until that ships in the version the customer is pinned to:
...
- `gateway_origin`: `direct` (Ratel called from library code) vs `agent` (Ratel called via the `search_tools` / `invoke_tool` gateway MCP tools). Critical pivot — direct calls don't reflect agent behaviour.
...
| `top_k_recall_at_5` | every `ratel.search_tools` observation with ground truth | 1 if `ground_truth_tool_id` appears in the top 5 hits, else 0 |
