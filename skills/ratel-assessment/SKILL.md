...
1. Assemble a JSON payload carrying the same content — `partner`, `date`, `stack`, `scope`, `data_sources`, `overall_score`, `summary`, the twelve `dimensions` (`{name, label, score}` in catalog order), optional `category_takeaways` (a `{category name → one-line "so what"}` map; the renderer rolls the twelve dimensions into four scored categories — Architecture & orchestration, Context & tools, Reliability & safety, Operations — and this is the high-level "what to improve" line under each, which should point to the specific findings below), the `findings` array (`title`, `dimension`, `severity`, `evidence`, `rationale`, `recommendation`, optional `ratel_angle`), and optional `where_ratel_fits`, `next_steps`, `appendix`. The schema is worked out in full in [`assets/sample-payload.json`](assets/sample-payload.json). Content fields may use inline `` `code` `` and `*emphasis*`; the renderer converts them and HTML-escapes everything else.
