---
name: ratel-decompose-prompt
description: |
  Take an agent's long, monolithic system prompt — the kind that mixes role, tool docs, output contracts, few-shot examples, and a dozen recurring procedures into one wall of text the model pays for every turn — and break it into a lean, stable core prompt plus a set of extractable Ratel-style skills (reusable Markdown playbooks) registered in a SkillCatalog and surfaced on demand via `search_capabilities`, so each procedure enters context only when it's relevant instead of every turn. Use whenever the user wants to shrink or restructure a bloated prompt, asks "break down my system prompt", "my system prompt is too long", "extract skills from this prompt", "split the prompt into skills", "turn prompt sections into Ratel skills", "decompose this prompt", "my prompt does too many things", or invokes `/ratel-decompose-prompt`. Trigger on phrases like "this system prompt is a monster", "factor out the procedures from my prompt", "move these instructions into skills", "the model reads this whole thing every call" — even if the user doesn't say "skill" or "ratel-decompose-prompt" by name. This skill writes a markdown decomposition plan; it does not edit the agent's prompt or code itself.
allowed-tools:
  - Bash
  - Read
  - Write
