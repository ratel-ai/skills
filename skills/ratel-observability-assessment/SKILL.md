...
description: |
  Inspect an agent codebase, decide where observability belongs and which dashboards would prove value, detect (or ask for) the AI-observability vendor in use, and write a vendor-neutral proposal that routes to the matching vendor integrate skill. Use whenever the user wants to mount observability for the first time, asks "where would tracing go in this codebase", "what should we instrument", "which dashboards should we build", "what should we measure", "set up observability here", "we're on Langfuse / LangSmith / PostHog — what next", or invokes `/ratel-observability-assessment`. Trigger on phrases like "add observability", "we just signed a partner, let's get tracing in", "instrument this agent", "trace coverage for this agent", "design our agent dashboards", "what proves out Ratel's value here" — even if the user doesn't say "skill" or "ratel-observability-assessment" by name. This skill writes a markdown proposal; it does not edit the agent code itself and does not call any vendor API.
allowed-tools:
  - Bash
  - Read
  - Write
