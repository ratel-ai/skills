...
description: |
  Read a customer's live LangSmith runs, traces, threads, and feedback scores through the LangSmith MCP server (the OAuth Remote MCP at api.smith.langchain.com/mcp or the standalone langsmith-mcp-server), look for issues and improvement opportunities, and write a findings report split into Ratel-flavored improvements (we fix this by integrating or deepening Ratel) and stack-agnostic low-hanging fruit (anyone could fix it; we earn trust by spotting it). Use whenever the user asks "look at our LangSmith runs", "analyze our LangSmith traces", "what's wrong with our agent", "find improvements in LangSmith", "review the runs in our project this week", "any low-hanging fruit in the trace data", "dig into the run tree", or invokes `/ratel-langsmith-analyze`. Trigger on phrases like "scan the LangSmith dashboard", "go through the run trees", "review their LangSmith observability", "dig into the failed runs", "what's the agent doing wrong in LangSmith" — even if the user doesn't say "skill" or "ratel-langsmith-analyze" by name. Assumes the LangSmith MCP server is reachable (set up during `/ratel-langsmith-integrate`), with a `langsmith` SDK fallback for offline analysis. Be honest when the data is clean — don't manufacture findings. Writes a markdown findings report to `<repo>/.ratel/`; it does not edit agent code.
allowed-tools:
  - Bash
  - Read
  - Write
