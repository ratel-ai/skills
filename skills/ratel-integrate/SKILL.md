...
description: |
  Inspect a customer codebase, figure out how it manages tools and which agent framework it uses, fetch the up-to-date Ratel documentation, and propose a concrete plan to integrate Ratel's tool-search gateway (BM25 retrieval + `search_tools` / `invoke_tool` gateway tools). The plan includes the integration mode (direct SDK vs MCP gateway vs hybrid), an A/B test design so the partner can measure the impact, and the Langfuse metrics that will prove the integration is working. Use whenever the user wants to add Ratel to a partner codebase, asks "wire up Ratel here", "how would Ratel fit", "integrate the gateway", "add tool search", "Ratel pilot for this customer", or invokes `/ratel-integrate`. Trigger on phrases like "let's add Ratel", "integrate the context gateway", "set up the Ratel SDK in this repo", "plan a Ratel rollout for them", "Ratel A/B for this agent" — even if the user doesn't say "skill" or "ratel-integrate" by name. This skill writes a markdown plan and asks clarifying questions when the codebase is ambiguous; it does not edit the agent code itself.
allowed-tools:
  - Bash
  - Read
  - Write
